...
export const slide = mysqlTable("slide", {
    ...
    text: text("text"),
    header: varchar("header", { length: 225 }),
...
export const produk = mysqlTable("produk", {
    ...
    brand: varchar("brand", { length: 225 }),
    gender: varchar("gender", { length: 225 }),
...
export const produkDetail = mysqlTable("produkdetail", {
    ...
    hargaSuperGold: int("harga_super_gold"),
    hargaSubAgen: int("harga_sub_agen"),
    hargaMarketer: int("harga_marketer"),
...
export const voucherHistory = mysqlTable("voucher_history", {
    ...
    kodeVoucher: varchar("kode_voucher", { length: 50 }).notNull(),
    namaVoucher: varchar("nama_voucher", { length: 100 }).notNull(),
    tanggalMulai: timestamp("tanggal_mulai").notNull(),
    tanggalKadaluarsa: timestamp("tanggal_kadaluarsa").notNull(),
    nilaiVoucher: int("nilai_voucher").notNull(),
    tipeVoucher: varchar("tipe_voucher", { length: 20 }).notNull(),
    orderTipe: varchar("order_tipe", { length: 20 }).notNull(),
    kategoriCustomerId: varchar("kategori_customer_id", { length: 20 }).notNull(),
    minimalTransaksi: int("minimal_transaksi").notNull(),
    custId: text("cust_id"),
    kuotaVoucher: int("kuota_voucher").notNull(),
    deskripsiVoucher: text("deskripsi_voucher").notNull(),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").notNull(),
    createdBy: int("created_by").notNull(),
    updatedBy: int("updated_by").notNull(),
    isAktif: int("is_aktif").notNull().default(0),
    maksimalNominalVoucherPersen: int("maksimal_nominal_voucher_persen").notNull(),
    syaratDanKetentuan: text("syarat_dan_ketentuan").notNull(),
});
...
export const payment = mysqlTable("payment", {
    ...
    expiredTime: timestamp("expired_time"),
    subtotal: int("subtotal"),
    otherFee: int("other_fee"),
    postFee: int("post_fee"),
    ammount: int("ammount"),
    voucherNominal: int("voucher_nominal").notNull().default(0),
    uniqueCode: varchar("unique_code", { length: 10 }),
    ammountTotal: int("ammount_total"),
    ammountTotalWallet: int("ammount_total_wallet").notNull().default(0),
    paymentTransactionTypeId: int("payment_transaction_type_id"),
    paymentTransactionId: varchar("payment_transaction_id", { length: 100 }),
    createdAt: timestamp("created_at"),
    createdBy: int("created_by"),
    providerMutationId: varchar("provider_mutation_id", { length: 50 }),
    providerBankId: varchar("provider_bank_id", { length: 50 }),
    providerAccountNumber: varchar("provider_account_number", { length: 50 }),
    providerBankType: varchar("provider_bank_type", { length: 50 }),
    providerDate: varchar("provider_date", { length: 50 }),
    providerAmmount: int("provider_ammount"),
    providerDescription: text("provider_description"),
    providerType: varchar("provider_type", { length: 5 }),
    tujuanAtasNama: varchar("tujuan_atas_nama", { length: 50 }),
    tujuanNamaBank: varchar("tujuan_nama_bank", { length: 50 }),
    tujuanLogoBank: varchar("tujuan_logo_bank", { length: 100 }),
    tujuanNoRekening: varchar("tujuan_no_rekening", { length: 50 }),
    paymentType: varchar("payment_type", { length: 50 }).notNull().default(""),
    isDp: int("is_dp").notNull().default(0),
    voucherKode: varchar("voucher_kode", { length: 50 }).notNull().default(""),
    isDeleted: int("is_deleted").notNull().default(0),
    isPaidBy: varchar("is_paid_by", { length: 50 }),
